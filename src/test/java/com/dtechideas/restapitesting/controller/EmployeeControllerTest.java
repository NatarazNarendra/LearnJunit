...
public class EmployeeControllerTest {
	 ...
	 * @Test
	 * 
	 * @DisplayName("JUnit test for getAllEmployees operation") public void
	 * givenEmployeesList_whenGetAllEmployees_thenReturnEmployeesList() throws
	 * Exception { // given - precondition or setup Employee employee1 = new
	 * Employee(1L,"Richard", "Parker", "richard.parker@dtechideas.com"); Employee
	 * employee2 = new Employee(2L,"Peter", "Parker",
	 * "peter.parker@dtechideas.com"); List<Employee> employees = List.of(employee1,
	 * employee2); given(employeeService.getAllEmployees()).willReturn(employees);
	 * // when - action or the behaviour ResultActions response =
	 * mockMvc.perform(get("/api/v1/employees")); // then - verify the output
	 * response.andDo(print()).andExpect(status().isOk()).andExpect(jsonPath(
	 * "$.size()", is(employees.size()))); }
	 */
